...
Movie.deleteMany()
	.then(() => console.log('Wiped Movies'))
	.then(() => Movie.insertMany(movies))
	.then(dbMovies => console.log(`Inserted ${dbMovies.length} movies into DB`))
	.then(() => Movie.find({}, 'title'))
	.then((dbMovies) => console.log(`Movies in DB are ${dbMovies.map(movie => `"${movie.title}"`).join(', ')}.`))
